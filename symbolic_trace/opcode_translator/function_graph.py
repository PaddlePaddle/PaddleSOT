...
from .variables import VariableTracker, TensorVariable, VariableTrackerFactory
from ..proxy_tensor import ProxyTensorContext, callable_wrapper, ProxyTensor
...
class FunctionGraph: 
    ...
    def start_compile(self, ret_val):
        ...
        for name in input_names:
            for tracker in self.input_trackers:
                if (isinstance(tracker, TensorVariable) and 
                    tracker.value.name == name):
                    self.pycode_gen.add_pure_instructions(tracker.source.gen_instructions())
        # call the compiled_fn
        self.pycode_gen.gen_call_function(
            argc=1
        )
        # restore the outputs.
    ...
    def call_paddle_api(self, func, *args, **kwargs):
        ...
        args, kwargs = self.collect_input_trackers(args), self.collect_input_trackers(kwargs)
